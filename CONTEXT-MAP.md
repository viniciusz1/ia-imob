...
- [Novos Imóveis](./docs/contexts/new-properties/CONTEXT.md) — validates and defines the future Agency-facing flow for reviewing listings that first appear in a Published Snapshot.
...
- **Novos Imóveis -> Crawler Machine**: Novos Imóveis classifies a market listing as new only when its stable Listing Identity first appears in a Published Snapshot; changed or reappearing versions of the same identity are not new listings.
