...
Public property detail pages and category/landing pages are rendered with Next.js ISR (statically generated, then revalidated) for SEO and speed. Cache freshness is driven by **on-demand revalidation**: when Laravel observes a `Property` change (publish, unpublish, update) or a Agency's branding change, it fires a **signed webhook** to a Next route (`POST /api/revalidate`) that calls `revalidateTag` on tags such as `agency:{id}` and `property:{id}`. Search/filter result pages are rendered SSR (dynamic), since filter combinations are unbounded and are not the indexable SEO surface.
...
- Cache tagging convention (`agency:{id}`, `property:{id}`) must be applied consistently in the public fetch layer.
