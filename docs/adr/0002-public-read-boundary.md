...
We chose this over reusing the authenticated property endpoint because the obvious shortcut — "just call the existing endpoint" — would leak internal CRM data and exact locations to anonymous visitors and bypass the published/agency filters. Keeping a separate read model makes the safe path the only path.
