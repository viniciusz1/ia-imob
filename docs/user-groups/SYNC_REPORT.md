...
| Criar Role (request) | `name` obrigatório, `permissions` array obrigatório (IDs válidos) | `roleSchema`: `name` obrigatório, `permissions` array obrigatório | OK |
...
3. Fixar no PRD de `user-groups` a convenção final (sem “ou”):
...
- Pontos críticos bloqueantes: 1 ([ERRO] nomenclatura do vínculo `group_id` vs `role_id`).
- Pontos não bloqueantes: 2 ([AVISO] `label`, `created_at`).

Status final: **NÃO APROVADO para integração final** até resolver o contrato do identificador de grupo/role no payload de usuário.
