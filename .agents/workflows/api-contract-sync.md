...
Description: Este workflow garante a integridade dos dados entre o backend (Laravel) e o frontend (Next.js) analisando as definições técnicas nos arquivos de PRD.
Trigger: Comando manual "Validar sincronização do módulo [nome]" ou alteração detectada nos arquivos dentro da pasta PRD/{nome-modulo}/.
...
Missing Fields: Campos presentes em um PRD mas ausentes no outro.
...
Crie ou atualize o arquivo PRD/{nome-modulo}/SYNC_REPORT.md.
...
Finalization: Notifique o usuário sobre o estado da sincronização. Não altere arquivos de código fonte (.php, .ts, .tsx); limite-se à documentação técnica no diretório PRD/.