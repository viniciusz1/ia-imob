...
import { listCrawlRuns, listCrawlerOperations, listDiscoverySnapshots, listExtractionProfiles } from "@/services/crawlerService";
...
export default async function CrawlAgencyDetailPage({ params }: CrawlAgencyDetailPageProps) {
  ...
  const [agency, snapshots, profiles, runs, schedule, operations] = await Promise.all([
  ...
  return <CrawlAgencyWorkspaceClient agency={agency} initialOperations={operations} profiles={profiles} runs={runs} schedule={schedule} snapshots={snapshots} />;
}
